...
async function handleRequest(req, res, projectDir, globalDir, client, defaultModel, systemPrompt, chatSystemTemplate, dirname) {
  ...
  // 3e. DELETE /api/workflows/:name — delete workflow from project
  if (method === 'DELETE' && parts[1] === 'api' && parts[2] === 'workflows' && parts.length === 4) {
    const name = parts[3];
    try {
      const resolved = await resolveWorkflowPath(projectDir, globalDir, name);
      if (!resolved) { sendError(res, 404, `Workflow "${name}" not found`); return; }
      if (resolved.source !== 'project') { sendError(res, 403, 'Only project workflows can be deleted'); return; }
  ...
  // 6. POST /api/workflows/:name/chat — chat with tool_use loop
...
export async function createServer_(opts = {}) {
  ...
  const server = createServer(async (req, res) => {
    try {
      await handleRequest(req, res, projectDir, globalDir, client, defaultModel, systemPrompt, chatSystemTemplate, __dirname);
    } catch (err) {
      sendError(res, 500, err.message);
    }
  });
