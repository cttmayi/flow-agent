...
function createAnthropicClient(config = {}) {
  return new Anthropic({
    apiKey: config.anthropic_api_key || process.env.ANTHROPIC_API_KEY || undefined,
    baseURL: config.base_url || process.env.ANTHROPIC_BASE_URL || undefined,
  });
}
...
async function handleRequest(req, res, projectDir, globalDir, client, defaultModel, systemPrompt, chatSystemTemplate, dirname, createRegistry, config) {
  ...
  if (method === 'POST' && parts[1] === 'api' && parts[2] === 'workflows' && parts[3] === 'create' && parts.length === 4) {
    try {
      ...
      const isAutoPhase = ['generate', 'review', 'verify'].includes(phase);
      // Auto-advance phases always get context; interactive phases get it on first message
      const shouldInject = hasPriorContext && (isAutoPhase || session.messages.length <= 1);
      if (shouldInject) {
  ...
  if (method === 'POST' && parts[1] === 'api' && parts[2] === 'workflows' && parts[4] === 'run' && parts.length === 5) {
    const name = parts[3];
    // Read body for args before setting up SSE
    let args = '';
    try {
      const body = await readBody(req);
      args = body?.args || '';
    } catch { /* body read failed, proceed without args */ }

    const session = getSession(name);
    const userMsg = args
      ? `执行 workflow "${name}"，参数：${args}`
      : `执行 workflow "${name}"`;
...
export async function createServer_(opts = {}) {
  ...
  createRegistry.register(fetchTool);
  createRegistry.register(searchTool);
