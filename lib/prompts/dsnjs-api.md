...
- "fetch": 获取网页内容。参数 { url: string }。输入 URL 返回页面纯文本（HTML 已转 markdown）。
- "search": 搜索网络。参数 { query: string, count?: number }。返回搜索结果列表（标题、链接、摘要）。
...
- args — 全局字符串变量，工作流执行时传入的参数（通过聊天输入框中斜杠命令 `/workflow名称 参数...` 传入）。如果没有参数则为空字符串 `""`。
  - ⚠️ args 已经是全局变量，直接使用，禁止 `const args =` 或 `let args =` 重复声明。
  - 示例：输入 `/translate Hello world` 执行时，workflow 中直接访问 `args` 值为 `"Hello world"`。
  - 典型用法：`await agent(\`翻译以下内容：${args}\`, { tools: ['bash'] })`
