// lib/api/agent-spawn.js — Generic CLI subprocess agent provider
import { spawn } from 'node:child_process';

function convertFormat(text, format) {
  if (format === 'text' || !format) return text;
  if (format === 'json') {
    const trimmed = text.trim();
    try { return JSON.parse(trimmed); } catch {}
    const match = trimmed.match(/```(?:json)?\s*\n?([\s\S]*?)```/);
    if (match) {
      try { return JSON.parse(match[1].trim()); } catch {}
    }
    throw new Error('输出不是合法的 JSON');
  }
  if (format === 'code') {
    const trimmed = text.trim();
    const match = trimmed.match(/```[\w]*\n?([\s\S]*?)```/);
    if (match) return match[1].trim();
    return trimmed;
  }
  return text;
}

export function createSpawnAgent(spawnConfig) {
  const { command, args: argTemplate, timeout: defaultTimeout, tool_mapping: toolMapping } = spawnConfig;

  if (!command) {
    throw new Error('Spawn agent missing required "command" config');
  }

  return async (prompt, agentOpts = {}) => {
    // Build tools string with mapping
    let toolsStr = '';
    if (agentOpts.tools && agentOpts.tools.length > 0) {
      const mapped = agentOpts.tools.map(t => (toolMapping && toolMapping[t]) || t);
      toolsStr = mapped.join(',');
    }

    const timeout = agentOpts.timeout || defaultTimeout || 120000;
    const maxTokens = agentOpts.maxTokens || 64 * 1024;

    // Template substitution
    let args = (argTemplate || []).map(a =>
      a.replace('{prompt}', prompt).replace('{tools}', toolsStr).replace('{timeout}', String(timeout)).replace('{max_tokens}', String(maxTokens))
    );

    // Replace empty tool values with explicit "" so claude sees --tools ""
    // instead of --tools with no argument (which claude would ignore)
    const filtered = [];
    for (let i = 0; i < args.length; i++) {
      if (args[i] === '' && i > 0 && args[i - 1] === '--tools') {
        filtered.push('""');
      } else if (args[i] !== '') {
        filtered.push(args[i]);
        // claude CLI 需要 --allowedTools 才能实际执行工具
        if (args[i - 1] === '--tools' && args[i] !== '""') {
          filtered.push('--allowedTools', args[i]);
        }
      }
    }
    args = filtered;

    return new Promise((resolve, reject) => {
      let settled = false;

      const child = spawn(command, args, {
        stdio: ['ignore', 'pipe', 'pipe'],
        timeout,
        env: {
          ...process.env,
          BASH_DEFAULT_TIMEOUT_MS: String(timeout),
          BASH_MAX_TIMEOUT_MS: String(timeout),
          API_TIMEOUT_MS: String(timeout),
          MAX_TOKENS: String(maxTokens),
        },
      });

      let stdout = '';
      let stderr = '';

      child.stdout.on('data', (data) => {
        const text = data.toString();
        stdout += text;
        if (agentOpts.onProgress) agentOpts.onProgress(text);
      });
      child.stderr.on('data', (data) => {
        const text = data.toString();
        stderr += text;
        if (agentOpts.onProgress) agentOpts.onProgress(text);
      });

      child.on('error', (err) => {
        if (!settled) { settled = true; reject(new Error(`Failed to spawn "${command}": ${err.message}`)); }
      });

      child.on('timeout', () => {
        if (!settled) { settled = true; reject(new Error(`"${command}" timed out after ${timeout / 1000}s`)); }
        child.kill('SIGTERM');
      });

      child.on('close', (code) => {
        if (settled) return;
        if (code !== 0) {
          settled = true;
          const msg = [`"${command}" 退出码 ${code}`];
          if (stdout.trim()) msg.push(`stdout: ${stdout.trim().slice(0, 500)}`);
          if (stderr.trim()) msg.push(`stderr: ${stderr.trim().slice(0, 500)}`);
          reject(new Error(msg.join('\n')));
        } else {
          settled = true;
          let result = stdout.trim();
          const format = agentOpts.format;
          if (format && format !== 'text') {
            try {
              result = convertFormat(result, format);
            } catch (err) {
              reject(new Error(`agent() ${format} 格式转换失败: ${err.message}\n原始返回：\n${result.slice(0, 500)}`));
              return;
            }
          }
          resolve(result);
        }
      });
    });
  };
}
