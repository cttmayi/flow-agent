...
export function createAgent(toolRegistry, opts = {}) {
  return async (prompt, agentOpts = {}) => {
    const model = (agentOpts.model && agentOpts.model !== 'auto') ? agentOpts.model : (opts.defaultModel || 'claude-sonnet-4-20250514');
    ...
    try {
      ...
      while (toolUseCount < MAX_TOOL_ROUNDS) {
        ...
        // Report intermediate text to caller if they want progress
        if (onProgress) {
